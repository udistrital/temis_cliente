...
export class CrudExperienciaLaboralComponent implements OnInit {
  ...
  construirForm() {
    ...
    this.route.queryParams
      ...
      .subscribe(params => {
        ...
        if (this.id != null)
          this.http.get('http://localhost:8080/v1/experiencia_laboral/' + (this.id).toString(), {
  ...
  updateInfoExperienciaLaboral(): void {
  ...
  createInfoExperienciaLaboral(organizacion: number, fechaIni: string, fechaFin: string, horas: number, dias: number): void {
    let url = 'http://localhost:8080/v1/experiencia_laboral/' + (this.id != null ? this.id : '')
    console.log(url)

    let data = {
      DiasLaboradosSemana: Number(dias),
      EntidadId: Number(organizacion),
      FechaIngreso: new Date(fechaIni),
      FechaRetiro: new Date(fechaFin),
      HorasLaboradasDia: Number(dias),
    ...
    this.http.post(url, data, {
      ...
      if (res != null) {
        console.log(res)
        this.router.navigate(['/pages/experiencia_laboral/experiencia_laboral-list'], { queryParams: { usuarioId: this.usuarioId } })
      }
    })
